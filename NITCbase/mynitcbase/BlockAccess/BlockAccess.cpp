...
int test = 0;
RecId BlockAccess::linearSearch(int relId, char attrName[ATTR_SIZE], union Attribute attrVal, int op, int* comp) {
    RecId prevRecId;
...
int BlockAccess::renameRelation(char oldName[ATTR_SIZE], char newName[ATTR_SIZE]){
    ...
    RecId ls = linearSearch(RELCAT_RELID, RELCAT_ATTR_RELNAME, newRelationName,EQ, &test);
    ...
    ls = linearSearch(RELCAT_RELID, RELCAT_ATTR_RELNAME, oldRelationName,EQ, &test);
    if(ls==(RecId){-1,-1}){
        return E_RELNOTEXIST;
    }
    ...
    */
    ...
    for(int i = 0; i < relRecord[RELCAT_NO_ATTRIBUTES_INDEX].nVal; i++){
        ls = linearSearch(ATTRCAT_RELID, ATTRCAT_ATTR_RELNAME, oldRelationName, EQ, &test);
...
int BlockAccess::renameAttribute(char relName[ATTR_SIZE], char oldName[ATTR_SIZE], char newName[ATTR_SIZE]) {
    ...
    //    return E_RELNOTEXIST;
    RecId ind = linearSearch(RELCAT_RELID, RELCAT_ATTR_RELNAME, relNameAttr, EQ, &test);
    if(ind==(RecId){-1,-1}){
        return E_RELNOTEXIST;
    }
    ...
    while (true) {
        ...
        RecId index = linearSearch(ATTRCAT_RELID, ATTRCAT_ATTR_RELNAME, relNameAttr, EQ, &test);
...
int BlockAccess::search(int relId, Attribute *record, char attrName[ATTR_SIZE], Attribute attrVal, int op, int* comp) {
    ...
    if (rootBlock == -1){
        ...
        */
       recId = linearSearch(relId, attrName, attrVal, op, comp);
    }
    ...
    /* else */ 
        ...
        recId = BPlusTree::bPlusSearch(relId, attrName, attrVal, op, comp);
    }
...
int BlockAccess::deleteRelation(char relName[ATTR_SIZE]) {
    ...
    RecId relCatRecId = BlockAccess::linearSearch(RELCAT_RELID, RELCAT_ATTR_RELNAME,relNameAttr, EQ, &test);
    ...
    ***/
    ...
    while(true) {
        ...
        attrCatRecId = BlockAccess::linearSearch(ATTRCAT_RELID, RELCAT_ATTR_RELNAME, relNameAttr, EQ, &test);
